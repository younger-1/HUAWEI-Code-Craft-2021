...
void generateServer(string &serverType, string &cpuCores, string &memorySize,
    ...
    serverInfos[_serverType] =
        vector<int>{_cpuCores / 2,   _cpuCores / 2, _memorySize / 2,
                    _memorySize / 2, _serverCost,   _powerCost};
}

void generateVm(string &vmType, string &vmCpuCores, string &vmMemory,
                string &vmTwoNodes) {
    string _vmType = "";
...
// 尝试在服务器上分配虚拟机资源
bool choseServer(vector<int> &server, vector<int> &vm, int serverId,
                 string vmId) {
    int vmCores = vm[0], vmMemory = vm[1], vmTwoNodes = vm[2];
    int &serverCoreA = server[0], &serverCoreB = server[1],
        &serverMemoryA = server[2], &serverMemoryB = server[3];
    if (vmTwoNodes) {
        int needCores = vmCores / 2, needMemory = vmMemory / 2;
        if (serverCoreA >= needCores && serverCoreB >= needCores &&
            serverMemoryA >= needMemory && serverMemoryB >= needMemory) {
...
int createVM(vector<string> &createVmInfo) {
    ...
    for (int i = 0; i < serverNumber; i++) {
        ...
        assert(server[0] >= 0 && server[1] >= 0 && server[2] >= 0 &&
               server[3] >= 0);
    }
    return success;
}
...
// 扩容服务器策略
void expansion() { cout << "(purchase, 0)\n"; }

// 迁移虚拟机策略
void migrate() { cout << "(migration, 0)\n"; }
